...
public class EnemyPathfinding : MonoBehaviour
{
	...
	public void SetDivisionConfiguration(DivisionConfiguration divConfig, DivisionSpawn divisionSpawn, int id = 0)
	{
		divSpawn = divisionSpawn;
	...
	IEnumerator AgentChanceToReact()
	{
		while (!isReacting)
		{
			...
			if (UnityEngine.Random.Range(1, 100) <= aiChanceToReact)
			{
				if (isChasingPlayer)
				{
					StartingState(EnemyMovingState.ChasingPlayer);
				}
				else
				{
